			...
			>
				<div className="relative overflow-hidden">
					<div
						...
						onMouseUp={stopDrag}
						className="p-6 flex gap-6 md:gap-8 text-black overflow-x-auto no-scrollbar overflow-y-hidden hide-scrollbar cursor-grab active:cursor-grabbing scroll-smooth"
					>
						{fits.map((fit, index) => (
							<motion.div
								key={fit.id}
								onClick={() => handleCardClick(fit.title, index)}
								className={`h-full w-full flex-shrink-0 rounded-lg shadow-lg relative border-2 transition-all duration-300 ${
									currentSlide === index
										? "border-black"
										: "border-transparent"
								}`}
